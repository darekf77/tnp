...
export class Framework extends BaseFeatureForProject<Project> {
  ...
  fixCoreContent = (
    ...
    const additionalSuffix = additionalEntrypointId
      ? `Server${additionalEntrypointId}`
      : '';
    const project = this.project;
    const coreName = _.upperFirst(_.camelCase(project.name + additionalSuffix));
    const coreNameKebab = _.kebabCase(project.name + additionalSuffix);
    return appTsContent
      .replace(
        new RegExp(
  ...
  recreateFileFromCoreProject = (options: {
    ...
    modifyContentBeforeSave?: (content: string) => string;
  }): void => {
    //#region @backendFunc
    let {
      fileRelativePath,
    ...
    if (
      ...
      let fixedContent = this.fixCoreContent(
        sourceContent,
        additionalEntrypointId,
      );
      if (modifyContentBeforeSave) {
        fixedContent = modifyContentBeforeSave(fixedContent);
      }
  ...
  contextFilter(relativePath: string): boolean {
    return (
      ...
      relativePath.startsWith(`${appTsFromSrc.replace('.ts', '.server')}`) ||
        ...
        .filter(
          i =>
            ...
            isomorphicLibs.includes(
              i.replace(
                new RegExp(
                  Utils.escapeStringForRegEx(`/${srcFromTaonImport}`) + '$',
                ),
                '',
              ),
            ),
  ...
  get allDetectedExternalNPmDependenciesForNpmLibCode(): string[] {
    ...
    let displayList = Utils.uniqArray(
      allImports
        .filter(
          f =>
            f.type === 'import' ||
  ...
  public recreateAppTsPresentationFiles(): void {
    ...
    this.project.writeFile(
      ...
      `App **${this.project.nameForNpmPackage}**\n\n${
...
`,
    ...
    content = content
      ...
      .replace(
        new RegExp(
          Utils.escapeStringForRegEx('#' + 'end' + 'reg' + 'ion'),
          'g',
        ),
        `#\${'end'+'reg'+'ion'}`,
      );
...
export const AppTs${_.camelCase(this.project.nameForNpmPackage)} = \`${content}\`;
export default AppTs${_.camelCase(this.project.nameForNpmPackage)};`,
    );
    //#endregion
  }
  //#endregion
