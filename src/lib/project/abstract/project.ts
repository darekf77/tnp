...
export class Project extends BaseProject<Project, CoreModels.LibType> {
  ...
  //#region static / taon tag to checkout for current cli version
  ...
  private __npmRunNg: string = `npm-run ng`; // when there is not global "ng" command -> npm-run ng.js works
  public angularFeBasenameManager: AngularFeBasenameManager;
  public readonly artifactsManager = BaseArtifact.for(this);

  async todeletaefunctino() {
    // await this.artifactsManager.build(BuildOptions.from({
    //   buildType: 'lib',
    //   isForRelease: 'minor',
    // }));
  }
  ...
  public async release(releaseOptions: ReleaseOptions) {
    ...
    if (releaseOptions.shouldReleaseLibrary) {
      if (releaseOptions.releaseVersionBumpType === 'major') {
        ...
        this.__packageJson.save(
          `${releaseOptions.releaseVersionBumpType} version up`,
        );
...
`);
  ...
  private async __buildVscode(buildOptions: BuildOptions) {
    ...
    if (
      !buildOptions.watch &&
      buildOptions.targetApp !== 'npm-lib-pkg-and-cli-tool' // TODO
    ) {
      await Helpers.ncc(
        crossPlatformPath([
          tmpVscodeProjPath,
  ...
  async build(buildOptions?: BuildOptions) {
    ...
    if (buildOptions.targetApp === 'vscode-extension-plugin') {
    ...
    if (buildOptions.targetApp === 'angular-electron-app') {
