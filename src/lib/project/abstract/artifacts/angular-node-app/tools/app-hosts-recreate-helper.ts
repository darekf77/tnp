 ...
 */
export let HOST_URL${n ? `_${n}` : ''} =  ${this.prefixVarTemplate('HOST_URL', n, { isURL: true })} ('http://localhost:' + HOST_BACKEND_PORT${n ? `_${n}` : ''});
//#${'reg' + 'ion'} @${'bro' + 'wser'}
if(TAON_CLOUDFLARE_BACKEND_HOST && TAON_CLOUDFLARE_BACKEND_HOST !== 'undefined') {
 HOST_URL${n ? `_${n}` : ''} = TAON_CLOUDFLARE_BACKEND_HOST;
}
//#${'endreg' + 'ion'}
/**
...
`;
    ...
    let ngCloudfalreAppPortFirst: number | undefined;
    ...
    for (let i = 0; i < allDetectedContexts.length; i++) {
      ...
      //#region ng cloudflare app port
      const ngCloudflareAppPort =
        ONLY_ONE_FRONTEND_IN_DEV && ngCloudfalreAppPortFirst
          ? ngCloudfalreAppPortFirst
          : await this.APP_NG_SERVE_ARTIFACT_PORT_UNIQ_KEY(
              buildOptions.clone({
                build: {
                  cloudflare: true,
      ...
      ngCloudfalreAppPortFirst =
        ngCloudfalreAppPortFirst || ngCloudflareAppPort;

      //#endregion

      //#region ng websql electron port
      ...
      //#region ng normal electron port
      ...
      //#region write cloudflare app port
      //  UtilsTypescript.setValueToVariableInTsFile(
      //   this.project.pathFor('src/app.hosts.ts'),
      //   `${FRONTEND_NORMAL_APP_PORT}_` + (i + 1),
      //   this.prefixVarTemplate(FRONTEND_NORMAL_APP_PORT, i + 1) +
      ...
      // );
      this.project.writeFile(
        `${tmp_FRONTEND_CLOUDFLARE_APP_PORT}_${i + 1}`,
        '' + ngCloudflareAppPort,
      );
      //#endregion

      //#region write websql electron port
      ...
      //#region write normaal electron port
      UtilsTypescript.setValueToVariableInTsFile(
        this.project.pathFor(
...
`,
  ...
  async APP_NG_SERVE_ARTIFACT_PORT_UNIQ_KEY(
    ...
    const key =
      ...
      ` (${buildOptions.build.cloudflare ? 'cloudfalre' : buildOptions.build.websql ? 'websql' : 'normal'})` +
      ` # context=${num || 1} `;
  ...
  async NODE_BACKEND_PORT_UNIQ_KEY(
    ...
    buildOptions = EnvOptions.from(buildOptions);
    const key = `node ${buildOptions.build.cloudflare ? 'cloudflare-worker-backend' : 'backend'}${num ? ` (${num})` : ''}`;
    return await this.project.registerAndAssignPort(key, {
