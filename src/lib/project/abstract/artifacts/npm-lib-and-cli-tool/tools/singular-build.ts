...
export class SingularBuild extends BaseFeatureForProject<Project> {
  ...
  async createSingluarTargeProjFor(options: {
    ...
    const children = this.project.children.filter(
      c =>
        c.typeIs('isomorphic-lib') && c.framework.frameworkVersionAtLeast('v3'),
    );
    const smartContainerTargetProjPath =
      parent.artifactsManager.globalHelper.__getProxyProjPath(
    ...
    children.forEach(c => {
      ...
      if (
        !nonClient &&
        c.name !== this.project.framework.smartContainerBuildTarget.name
      ) {
        (() => {
          const source_lib = crossPlatformPath(
...
exports.default = start;`,
    ...
    (() => {
      const corepro = this.project.ins.by(
        'isomorphic-lib',
        targetContainerProject.framework.frameworkVersion,
      ) as Project;
    ...
    let smartContainerTargetProject = this.project.ins.From(
      smartContainerTargetProjPath,
    );
    parent.nodeModules.linkToProject(smartContainerTargetProject as any);
    await smartContainerTargetProject.init(
      InitOptions.from({
        purpose: 'initing dist project',
      }),
    ); // THIS CAUSE NOT NICE SHIT
  ...
  async initSingularBuild(
    ...
    for (let index = 0; index < children.length; index++) {
      ...
      await c.init(
        InitOptions.from({
          purpose: 'singular build lib init',
        }),
      );
    }
