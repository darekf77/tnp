  ...
  extends BaseFeatureForProject<Project>
{
  ...
  public async update(
    ...
    //#region @backendFunc
    Helpers.taskStarted(
      `${fromWatcher ? '(watcher) ' : ''}Updating environment config `,
    );
    ...
    Helpers.taskDone(
      `${fromWatcher ? '(watcher) ' : ''}Done updating environment config `,
    );
  ...
  private saveEnvironmentConfig(projectEnvConfig: EnvOptions): void {
    ...
    // );
    if (projectEnvConfig.release.targetArtifact === 'electron-app') {
      debugger;
    }
    const targetArtifact =
      projectEnvConfig.release.targetArtifact || 'npm-lib-and-cli-tool';

    let browserConfig: EnvOptions;
    if (
      this.project.framework.isStandaloneProject ||
      this.project.framework.isContainer
    ) {
      // Helpers.writeJson(tmpEnvironmentPath, projectEnvConfig);
      browserConfig = _.cloneDeep(projectEnvConfig);
      ...
      // Helpers.writeJson(tmpEnvironmentPathBrowser, browserConfig);
    ...
    if (this.project.framework.isStandaloneProject) {
      const browserConfigFileName = `env.${targetArtifact}.browser.ts`;
      const backendConfigFileName = `env.${targetArtifact}.backend.ts`;
      const backendConstants: string[] = [];
      const frontendConstants: string[] = [];
      walk.Object(
        browserConfig,
        (val, lodashPath, newValue) => {
          if (!_.isObject(val)) {
            frontendConstants.push(
              `export const ENV_BROWSER_` +
      ...
      this.project.writeFile(
        `src/lib/env/${browserConfigFileName}`,
        `${THIS_IS_GENERATED_INFO_COMMENT}
${frontendConstants.join('\n')}
${THIS_IS_GENERATED_INFO_COMMENT}`,
