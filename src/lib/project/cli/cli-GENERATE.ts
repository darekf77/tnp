...
import { isTsFile } from '../../app-utils';
import {
  appFromSrc,
...
export class $Generate extends BaseCli {
  ...
  async _() {
    ...
    const newEntityName = UtilsString.kebabCaseNoSplitNumbers(
      entityName?.trim(),
    );
    ...
    const placeholderFilesAbsPath = crossPlatformPath([
      exampleLocation,
      'placeholder',
    ]);

    const placeholderExcludeFilesAbsPath = crossPlatformPath([
      exampleLocation,
      'placeholder_exclude',
    ]);

    const filesToRecerateFromPlaceholder = !Helpers.exists(
      placeholderFilesAbsPath,
    )
      ? []
      : (
          UtilsFilesFoldersSync.readFile(placeholderFilesAbsPath, {
    ...
    const filesIgnore = !Helpers.exists(placeholderExcludeFilesAbsPath)
      ? []
      : (
          UtilsFilesFoldersSync.readFile(placeholderExcludeFilesAbsPath, {
            defaultValueWhenNotExists: '',
          }) || ''
        )
          .split('\n')
    ...
    // console.log({ filesIgnore });

    if (Helpers.exists(placeholderFilesAbsPath)) {
      UtilsFilesFoldersSync.writeFile(
        [path.dirname(placeholderFilesAbsPath), dotGitIgnoreMainProject],
        `/!placeholder
/**/*.ts
/**/*.tsx
/**/*.md
...
/**/*.scss
      ...
      UtilsFilesFoldersSync.getFilesFrom(
        path.dirname(placeholderFilesAbsPath),
        {
          recursive: false,
    ...
    // const taonFullAbsLocation = crossPlatformPath([
    //   container.location,
    //   `gen-examples-${container.framework.frameworkVersion}`,
    //   'taon-full',
    //   newEntityName,
    ...
    for (const placeholderRelativePath of filesToRecerateFromPlaceholder) {
      ...
      const dest = crossPlatformPath([
        path.dirname(placeholderFilesAbsPath),
        path.basename(placeholderRelativePath),
      ]);
      UtilsFilesFoldersSync.copyFile(source, dest);
    }
    ...
    if (isCustom) {
      ...
      //#region handle normal taon building blocks
...
import { MIGRATIONS_CLASSES_FOR_${_.upperFirst(_.camelCase(newEntityName))}ActiveContext }` +
    ...
    //#region regenerate autogenerated files
