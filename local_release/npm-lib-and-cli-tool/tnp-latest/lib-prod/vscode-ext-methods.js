import { crossPlatformPath, fse, path, LibTypeEnum, ___NS__merge, ___NS__startCase, Helpers__NS__exists, Helpers__NS__isFolder, Helpers__NS__mkdirp, Helpers__NS__readJson, Helpers__NS__writeJson, Utils__NS__camelize, UtilsOs__NS__getRealHomeDir } from 'tnp-core/lib-prod';
import { FrameworkUtils__NS__copyToAiContent, FrameworkUtils__NS__copyTree, FrameworkUtils__NS__pasteFromAIMDContentToFiles } from './framework-utils';
import { Project } from './project/abstract/project';
export const vscodeExtMethods = (FRAMEWORK_NAME) => {
    //#region @backendFunc
    ...
    const copyOrCutProject = async (action, vscode, uri) => {
        ...
        const currentContent = Helpers__NS__readJson(taonHomeDirForCopyPathJson);
    ...
    return [
        ...
        //#region PASTE PROJECT
        {
            ...
            async exec({ vscode, uri, rootFolderPath }) {
                //#region @backendFunc
                let MAIN_CLICKED_PATH = crossPlatformPath(uri.path) || crossPlatformPath(rootFolderPath);
                if (!MAIN_CLICKED_PATH) {
                    return;
                }
                if (!Helpers__NS__isFolder(MAIN_CLICKED_PATH)) {
                    MAIN_CLICKED_PATH = crossPlatformPath(path.dirname(MAIN_CLICKED_PATH));
                }
                if (!Helpers__NS__exists(taonHomeDirForCopyPathJson)) {
                    Helpers__NS__writeJson(taonHomeDirForCopyPathJson, {});
                }
                ...
                const copyOrMove = async (action) => {
                    const copyOrMoveSource = Project.ins.nearestTo(currentContent[action]);
                    if (!copyOrMoveSource) {
        ...
        },
        {
            ...
            exec: ({ vscode, uri, cwd }) => {
                const WORKSPACE_MAIN_FOLDER_PATH = crossPlatformPath(uri.path);
                vscode.env.clipboard.writeText(path.basename(WORKSPACE_MAIN_FOLDER_PATH));
            },
            options: {
                cancellable: false,
                showSuccessMessage: false,
        ...
        //#region GENERATE index.ts in selected folder
        {
            ...
            exec: async (data) => {
                ...
                vscode.env.clipboard.writeText(await FrameworkUtils__NS__copyTree(WORKSPACE_MAIN_FOLDER_PATH));
            },
            options: {
        ...
        },
        {
            ...
            exec: async (data) => {
                ...
                const WORKSPACE_MAIN_FOLDER_PATH = crossPlatformPath(uri.path);
                // vscode.window.showInformationMessage(WORKSPACE_MAIN_FOLDER_PATH);
                vscode.env.clipboard.writeText(await FrameworkUtils__NS__copyToAiContent(WORKSPACE_MAIN_FOLDER_PATH, arr));
            },
            options: {
                titleWhenProcessing: 'copying files and folders to clipboard MD/AI ready format',
                showSuccessMessage: false,
        ...
        },
        {
            ...
            exec: async ({ selectedUris, uri, vscode }) => {
                ...
                const processed = await FrameworkUtils__NS__pasteFromAIMDContentToFiles(WORKSPACE_MAIN_FOLDER_PATH, textFromClipboard);
                const nearestProject = Project.ins.nearestTo(WORKSPACE_MAIN_FOLDER_PATH);
                if (nearestProject) {
...
//# sourceMappingURL=c:/Users/darek/projects/npm/taon-dev/tnp/dist/lib-prod/vscode-ext-methods.js.map