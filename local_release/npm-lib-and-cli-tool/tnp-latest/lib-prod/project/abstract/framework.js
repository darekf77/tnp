//#region imports
import { backendNodejsOnlyFiles, config, extAllowedToExportAndReplaceTSJSCodeFiles, frontendFiles, LibTypeEnum, notNeededForExportFiles, TAGS, taonPackageName, UtilsTerminal__NS__confirm } from 'tnp-core/lib-prod';
import { crossPlatformPath, path, fse, ___NS__camelCase, ___NS__isString, ___NS__isUndefined, ___NS__kebabCase, ___NS__upperFirst, Helpers__NS__error, Helpers__NS__exists, Helpers__NS__filesFrom, Helpers__NS__foldersFrom, Helpers__NS__getFilesFrom, Helpers__NS__info, Helpers__NS__isFolder, Helpers__NS__readFile, Helpers__NS__readJsonC, Helpers__NS__taskDone, Helpers__NS__warn, Helpers__NS__writeFile, Utils__NS__escapeStringForRegEx, Utils__NS__uniqArray, UtilsOs__NS__isRunningInVscodeExtension } from 'tnp-core/lib-prod';
import { fileName } from 'tnp-core/lib-prod';
import { BaseFeatureForProject, UtilsNpm__NS__isProperVersion, UtilsTypescript__NS__getTaonContextsNamesFromFile, UtilsTypescript__NS__migrateFromNgModulesToStandaloneV21, UtilsTypescript__NS__recognizeImportsFromFile } from 'tnp-helpers/lib-prod';
import { createSourceFile, isClassDeclaration, ScriptTarget, forEachChild, } from 'typescript';
import { extractFirstLevelRegions, } from '../../app-utils';
import { appAutoGenDocsMd, appAutoGenJs, appTsFromSrc, containerPrefix, libFromSrc, migrationIndexAutogeneratedTsFileRelativeToSrcPath, nodeModulesMainProject, srcFromTaonImport, srcMainProject, TaonFileExtension, THIS_IS_GENERATED_INFO_COMMENT, tmpLocalCopytoProjDist, VERIFIED_BUILD_DATA, } from '../../constants';
import { EnvOptions } from '../../options';
//#endregion
// @ts-ignore TODO weird inheritance problem
export class Framework extends BaseFeatureForProject {
    ...
    get isContainerChild() {
        //#region @backendFunc
        return (!!this.project.parent && this.project.parent.typeIs(LibTypeEnum.CONTAINER));
        //#endregion
    }
    //#endregion
    ...
    get frameworkVersionMinusOne() {
        //#region @backendFunc
        const curr = Number(___NS__isString(this.frameworkVersion) &&
    ...
    migrateFromNgModulesToStandaloneV21(tsFileContent) {
        ...
        return UtilsTypescript__NS__migrateFromNgModulesToStandaloneV21(tsFileContent, ___NS__upperFirst(___NS__camelCase(this.project.name)));
    }
    replaceModuleAndComponentName(tsFileContent) {
        ...
        const sourceFile = createSourceFile('temp.ts', tsFileContent, ScriptTarget.Latest, true);
        let moduleName = null;
        let componentName = null;
        let tooMuchToProcess = false;
        const newComponentName = `${___NS__upperFirst(___NS__camelCase(projectName))}Component`;
    ...
    recreateVarsScss(initOptions) {
        ...
        this.project.writeFile('src/vars.scss', `${THIS_IS_GENERATED_INFO_COMMENT}
  ...
  `);
    ...
    recreateFileFromCoreProject = (options) => {
        ...
        if (customDestinationLocation &&
            ...
            Helpers__NS__error(`[${config.frameworkName}] Custom destination "${customDestinationLocation}"` +
                ` location cannot be a folder, it must be a file path`);
        }
        const project = this.project;
        fileRelativePath = crossPlatformPath(fileRelativePath);
        if ((___NS__isString(fileRelativePath) &&
    ...
    get containerDataFromNodeModulesLink() {
        //#region @backendFunc
        const realpathCCfromCurrentProj = fse.existsSync(this.project.nodeModules.path) &&
    ...
    get coreContainer() {
        ...
        const coreContainer = this.project.ins.by(LibTypeEnum.CONTAINER, this.frameworkVersion);
        if (!coreContainer) {
    ...
    generateIndexTs(relativePath = '') {
        ...
        const folders = [
            ...
            .filter(f => ___NS__isUndefined(notNeededForExportFiles.find(e => f.endsWith(e))))
            .filter(f => path.extname(f) === '' ||
            !___NS__isUndefined(extAllowedToExportAndReplaceTSJSCodeFiles.find(a => f.endsWith(a))));
        Helpers__NS__writeFile(crossPlatformPath([absPath, fileName.index_ts]), folders
            .map(f => {
            if (!___NS__isUndefined(frontendFiles.find(bigExt => f.endsWith(bigExt)))) {
                // `${TAGS.COMMENT_REGION} ${TAGS.BROWSER}\n` +
                return `export * from './${f.replace(path.extname(f), '')}'; // ${TAGS.BROWSER}`;
                // +`\n${TAGS.COMMENT_END_REGION}\n`;
            }
            if (!___NS__isUndefined(backendNodejsOnlyFiles.find(bigExt => f.endsWith(bigExt)))) {
                return (
    ...
    async global(globalPackageName, packageOnly = false) {
        //#region @backendFunc
        const oldContainer = this.project.ins.by(LibTypeEnum.CONTAINER, 'v1');
        if (oldContainer.nodeModules.empty) {
            Helpers__NS__info('initing container v1 for global packages');
            await oldContainer.init(EnvOptions.from({
                purpose: 'old container init',
            }));
        }
        if (packageOnly) {
            return crossPlatformPath(path.join(oldContainer.nodeModules.path, globalPackageName));
        }
        return crossPlatformPath(path.join(oldContainer.nodeModules.path, globalPackageName, `bin/${globalPackageName}`));
        //#endregion
    }
    //#endregion
    ...
    contextFilter(relativePath) {
        return (relativePath === appTsFromSrc ||
    ...
    _allDetectedNestedContexts(options) {
        ...
        const detectedDatabaseFiles = filesForContext.reduce((a, absPathToFile) => {
            return a.concat(UtilsTypescript__NS__getTaonContextsNamesFromFile(absPathToFile).map(contextName => {
                return {
    ...
    get allDetectedExternalIsomorphicDependenciesForNpmLibCode() {
        //#region @backendFunc
        const allFiles = Helpers__NS__getFilesFrom(this.project.pathFor([srcMainProject, libFromSrc]), {
            recursive: true,
        ...
        const displayList = Utils__NS__uniqArray(allImports
            .filter(f => f.type === 'import' ||
            f.type === 'require' ||
            f.type === 'async-import')
            .map(i => `${i.cleanEmbeddedPathToFile}`)
            .filter(i => !i.startsWith('.') &&
            isomorphicLibs.includes(i.replace(new RegExp(Utils__NS__escapeStringForRegEx(`/${srcFromTaonImport}`) + '$'), '')))
            .map(i => i.replace(new RegExp(Utils__NS__escapeStringForRegEx(`/${srcFromTaonImport}`) + '$'), ''))).sort();
        return displayList;
        //#endregion
    }
    //#endregion
    ...
    get allDetectedExternalNPmDependenciesForNpmLibCode() {
        ...
        let displayList = Utils__NS__uniqArray(allImports
            .filter(f => f.type === 'import' ||
            f.type === 'require' ||
            f.type === 'async-import')
            .map(i => `${i.cleanEmbeddedPathToFile}`)
...
`);
        content = content
            ...
            .replace(/\$/g, '\\$')
            .replace(/\@/g, `\${'@'}`)
            .replace(new RegExp(Utils__NS__escapeStringForRegEx('#' + 'reg' + 'ion'), 'g'), `#\${'reg'+'ion'}`)
            .replace(new RegExp(Utils__NS__escapeStringForRegEx('#' + 'end' + 'reg' + 'ion'), 'g'), `#\${'end'+'reg'+'ion'}`);
        this.project.writeFile([srcMainProject, appAutoGenJs], ` // This files is autogenerated and content is ready to be consumed in js.
export const AppTs${___NS__camelCase(this.project.nameForNpmPackage)} = \`${content}\`;
export default AppTs${___NS__camelCase(this.project.nameForNpmPackage)};`);
        //#endregion
    }
    //#endregion
    ...
    async setFrameworkVersion(newFrameworkVersion, options) {
        ...
        if (!newFrameworkVersion || !newFrameworkVersion.startsWith('v')) {
            Helpers__NS__error(`Invalid framework version: ${newFrameworkVersion}`, false, true);
        }
        ...
        options = options || {};
        Helpers__NS__info(`Setting framework version (${newFrameworkVersion}) for ${this.project.name}... and children`);
    ...
    async setNpmVersion(npmVersion, options) {
        ...
        options = options || {};
        Helpers__NS__info(`Setting npm version (${npmVersion}) for ${this.project.name}... and children`);
        const confirm = !options.confirm
            ? true
            : await UtilsTerminal__NS__confirm({
    ...
    async generateLibIndex() {
        ...
        Helpers__NS__info(`Library index.ts regenerated`);
        //#endregion
    }
    //#endregion
    ...
    filterVerfiedBuilds(packagesNames) {
        return packagesNames.filter(f => {
            ...
            return !Helpers__NS__readJsonC(pathToVerifyFile, {})
                ?.commitHash;
        });
    }
...
//# sourceMappingURL=c:/Users/darek/projects/npm/taon-dev/tnp/dist/lib-prod/project/abstract/framework.js.map