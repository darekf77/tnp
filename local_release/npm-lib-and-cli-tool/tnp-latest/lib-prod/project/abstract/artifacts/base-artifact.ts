...
> {
  ...
  protected __cutReleaseCodeFromSrc(buildOptions: EnvOptions) {
    ...
    filesForModyficaiton
      .filter(
        absolutePath =>
          !Helpers__NS__isFolder(absolutePath) &&
  ...
  > {
    ...
    if (Helpers__NS__exists([staticPagesProjLocation, taonJsonMainProject])) {
      HelpersTaon__NS__git__NS__cleanRepoFromAnyFilesExceptDotGitFolder(
    ...
    if (!releaseOptions.release.skipStaticPagesVersioning) {
      const versions = Helpers__NS__foldersFrom(
