    ...
    const result = InsideStruct.from(
      {
        ...
        endAction: async ({ replacement }) => {
          ...
          (() => {
            ...
            for (const destinationAbsPath of filesToProcess) {
              ...
              content = content.replace(
                new RegExp(Utils__NS__escapeStringForRegEx(fromReplace), 'g'),
                toReplace,
              );
          ...
          await (async () => {
            ...
            const manifestJson: CoreModels__NS__PwaManifest = Helpers__NS__readJson(
              manifestJsonPath,
              {},
              true,
            );
            ...
            if (this.project.artifactsManager.globalHelper.branding.exist) {
              ...
              const iconsFilesPathes = Helpers__NS__filesFrom(iconsPath).filter(
                f => {
                  return CoreModels__NS__ImageFileExtensionArr.includes(
