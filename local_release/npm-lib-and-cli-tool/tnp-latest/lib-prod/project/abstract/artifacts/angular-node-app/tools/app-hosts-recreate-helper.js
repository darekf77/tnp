import { ___NS__isEqual, ___NS__times } from 'tnp-core/lib-prod';
import { Helpers__NS__logInfo, Helpers__NS__taskDone, Helpers__NS__taskStarted, Helpers__NS__writeFile, UtilsTypescript__NS__exportsFromFile, UtilsTypescript__NS__setValueToVariableInTsFile } from 'tnp-helpers/lib-prod';
import { BaseDebounceCompilerForProject, } from 'tnp-helpers/lib-prod';
import { AngularJsonTaskName, appFromSrc, DEFAULT_PORT, FRONTEND_NORMAL_APP_PORT, FRONTEND_NORMAL_ELECTRON_PORT, FRONTEND_WEBSQL_APP_PORT, HOST_BACKEND_PORT, MIGRATION_CONST_PREFIX, migrationIndexAutogeneratedTsFileRelativeToSrcPath, srcMainProject, TaonCommands, TaonGeneratedFiles, THIS_IS_GENERATED_INFO_COMMENT, tmp_FRONTEND_NORMAL_APP_PORT, tmp_FRONTEND_NORMAL_ELECTRON_PORT, tmp_FRONTEND_WEBSQL_APP_PORT, tmp_HOST_BACKEND_PORT, USE_IN_HOST_CONFIG_FULL_CONTEXT_PATH, USE_MIGRATIONS_DATA_IN_HOST_CONFIG, } from '../../../../../constants';
...
export class AppHostsRecreateHelper extends BaseDebounceCompilerForProject {
    ...
    async rebuild(changeOfFiles, asyncEvent) {
        ...
        const migrationExported = USE_MIGRATIONS_DATA_IN_HOST_CONFIG
            ? UtilsTypescript__NS__exportsFromFile(this.project.pathFor([
                srcMainProject,
    ...
    writePortsToFile() {
        ...
        const contexts = this.lastTaonContexts.filter(f => f.fileRelativePath.startsWith(`${appFromSrc}/`) ||
...
//  (isWebSQLMode ? FRONTEND_WEBSQL_ELECTRON_PORT${n ? `_${n}` : ''} : FRONTEND_NORMAL_ELECTRON_PORT${n ? `_${n}` : ''});
  ...
  `);
        ...
        Helpers__NS__writeFile(appHostsFile, `
...
${contextsInFile
                .map((contextName, i) => {
                const contextMigraions = migrationExported.find(c => c.name.replace(MIGRATION_CONST_PREFIX, '') === contextName)?.name;
                ++counter;
                return `
...
`;
        ...
        const allDetectedContexts = this.project.framework
            ...
            .filter(f => f.fileRelativePath.startsWith(`${appFromSrc}/`) ||
            f.fileRelativePath.startsWith(`${appFromSrc}.`));
        ...
        for (let i = 0; i < allDetectedContexts.length; i++) {
            ...
            UtilsTypescript__NS__setValueToVariableInTsFile(this.project.pathFor('src/app.hosts.ts'), `${HOST_BACKEND_PORT}_` + (i + 1), this.prefixVarTemplate(HOST_BACKEND_PORT, i + 1) + nodeBeAppPort, {
                useRawStringValue: true,
            });
            this.project.writeFile(`${tmp_HOST_BACKEND_PORT}_${i + 1}`, '' + nodeBeAppPort);
            UtilsTypescript__NS__setValueToVariableInTsFile(this.project.pathFor('src/app.hosts.ts'), `${FRONTEND_WEBSQL_APP_PORT}_` + (i + 1), this.prefixVarTemplate(FRONTEND_WEBSQL_APP_PORT, i + 1) +
                ngWebsqlAppPort, {
                useRawStringValue: true,
            });
            this.project.writeFile(`${tmp_FRONTEND_WEBSQL_APP_PORT}_${i + 1}`, '' + ngWebsqlAppPort);
            UtilsTypescript__NS__setValueToVariableInTsFile(this.project.pathFor('src/app.hosts.ts'), `${FRONTEND_NORMAL_APP_PORT}_` + (i + 1), this.prefixVarTemplate(FRONTEND_NORMAL_APP_PORT, i + 1) +
                ngNormalAppPort, {
                useRawStringValue: true,
            });
            this.project.writeFile(`${tmp_FRONTEND_NORMAL_APP_PORT}_${i + 1}`, '' + ngNormalAppPort);
            ...
            //   ngWebsqlElectronPort,
            // );
            UtilsTypescript__NS__setValueToVariableInTsFile(this.project.pathFor(`${srcMainProject}/${TaonGeneratedFiles.APP_HOSTS_TS}`), `${FRONTEND_NORMAL_ELECTRON_PORT}_` + (i + 1), ngNormalElectronPort);
            this.project.writeFile(`${tmp_FRONTEND_NORMAL_ELECTRON_PORT}_${i + 1}`, '' + ngNormalElectronPort);
        }
        this.project.writeFile(TaonGeneratedFiles.BUILD_INFO_MD, `# CURRENT BUILD INFO
...
`);
    ...
    async APP_NG_SERVE_ARTIFACT_PORT_UNIQ_KEY(buildOptions, options) {
        ...
        const key = `${TaonCommands.NG} ${buildOptions.release.targetArtifact === ReleaseArtifactTaon.ELECTRON_APP
...
//# sourceMappingURL=c:/Users/darek/projects/npm/taon-dev/tnp/dist/lib-prod/project/abstract/artifacts/angular-node-app/tools/app-hosts-recreate-helper.js.map