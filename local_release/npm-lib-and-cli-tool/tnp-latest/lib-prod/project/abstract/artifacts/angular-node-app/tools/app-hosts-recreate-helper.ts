...
import {
  ...
  DOCKER_FOLDER,
  FRONTEND_NORMAL_APP_PORT,
  FRONTEND_NORMAL_ELECTRON_PORT,
  FRONTEND_WEBSQL_APP_PORT,
  HOST_BACKEND_PORT,
...
> {
  ...
  private lastTaonContexts: Models__NS__TaonContext[] = [];

  private lastMigrationExported: UtilsTypescript__NS__ExportInfo[] = [];
  ...
  constructor(project: Project) {
    ...
    this.baseSrcFolder = project.pathFor(srcMainProject) + '/';
  }
  //#endregion
  ...
  private async rebuild(
    ...
    Helpers__NS__taskStarted(`Rebuilding app.hosts.ts`);
    // if (asyncEvent) {
    ...
    const migrationExported = USE_MIGRATIONS_DATA_IN_HOST_CONFIG
      ? UtilsTypescript__NS__exportsFromFile(
    ...
    if (
      ...
      ___NS__isEqual(taonContexts, this.lastTaonContexts) &&
      (USE_MIGRATIONS_DATA_IN_HOST_CONFIG
        ? ___NS__isEqual(migrationExported, this.lastMigrationExported)
        : true)
    ) {
      Helpers__NS__logInfo(`No need for taon context update in app.hosts `);
      return;
    }
    ...
    Helpers__NS__taskDone(`Rebuilding app.hosts.ts Done`);
    //#endregion
  }
  ...
  async action({
    ...
    if (changeOfFiles.length === 0) {
      Helpers__NS__logInfo(`No file detected that affects app.hosts.ts`);
      return;
    }
  ...
  public writePortsToFile(): void {
    // #region @backend
    Helpers__NS__taskStarted('Writing hosts and ports config to app.hosts ...');
    ...
    const migrationExported: UtilsTypescript__NS__ExportInfo[] =
      this.lastMigrationExported;

    const contexts = this.lastTaonContexts.filter(
      f =>
        f.fileRelativePath.startsWith(`${appFromSrc}/`) ||
        f.fileRelativePath.startsWith(`${appFromSrc}.`),
    );
...
${___NS__times(contexts.length, i => {
  return tempalte(i + 1);
}).join('\n')}
${
  contexts.length > 0
...
${THIS_IS_GENERATED_INFO_COMMENT}
    ...
    Helpers__NS__taskDone('Done writing ports and hosts to app.hosts.ts');
    //#endregion
  }
  //#endregion
...
`;
    ...
    for (let i = 0; i < allDetectedContexts.length; i++) {
      ...
      this.project.writeFile(
        `${tmp_HOST_BACKEND_PORT}_${i + 1}`,
        '' + nodeBeAppPort,
      );

      UtilsTypescript__NS__setValueToVariableInTsFile(
        this.project.pathFor('src/app.hosts.ts'),
        `${FRONTEND_WEBSQL_APP_PORT}_` + (i + 1),
      ...
      this.project.writeFile(
        `${tmp_FRONTEND_WEBSQL_APP_PORT}_${i + 1}`,
        '' + ngWebsqlAppPort,
      );

      UtilsTypescript__NS__setValueToVariableInTsFile(
        this.project.pathFor('src/app.hosts.ts'),
        `${FRONTEND_NORMAL_APP_PORT}_` + (i + 1),
      ...
      this.project.writeFile(
        `${tmp_FRONTEND_NORMAL_APP_PORT}_${i + 1}`,
        '' + ngNormalAppPort,
      );
      ...
      UtilsTypescript__NS__setValueToVariableInTsFile(
        ...
        `${FRONTEND_NORMAL_ELECTRON_PORT}_` + (i + 1),
        ngNormalElectronPort,
      );
      this.project.writeFile(
        `${tmp_FRONTEND_NORMAL_ELECTRON_PORT}_${i + 1}`,
        '' + ngNormalElectronPort,
      );
    }
