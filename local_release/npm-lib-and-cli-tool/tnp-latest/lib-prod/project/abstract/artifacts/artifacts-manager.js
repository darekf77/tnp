//#region imports
import { config, dotTaonFolder, dotTnpFolder, GlobalStorage, LibTypeEnum, taonPackageName } from 'tnp-core/lib-prod';
import { chalk, crossPlatformPath, fse, path, ___NS__camelCase, Utils__NS__uniqArray, UtilsTerminal__NS__clearConsole, UtilsTerminal__NS__pressAnyKeyToContinueAsync, UtilsTerminal__NS__select } from 'tnp-core/lib-prod';
import { fileName } from 'tnp-core/lib-prod';
import { tnpPackageName } from 'tnp-core/lib-prod';
import { taonActionFromParent } from 'tnp-core/lib-prod';
import { Helpers__NS__error, Helpers__NS__exists, Helpers__NS__info, Helpers__NS__logWarn, Helpers__NS__remove, Helpers__NS__removeFileIfExists, Helpers__NS__taskDone, Helpers__NS__taskStarted, Helpers__NS__warn, Helpers__NS__writeFile, HelpersTaon__NS__copy, HelpersTaon__NS__git__NS__tagAndPushToGitRepo } from 'tnp-helpers/lib-prod';
import { CURRENT_PACKAGE_VERSION } from '../../../build-info._auto-generated_';
import { appElectronTsFromSrc, appTsFromSrc, containerPrefix, distMainProject, dotFileTemplateExt, dotVscodeMainProject, globalScssFromSrc, indexTsFromSrc, libFromSrc, migrationsFromLib, nodeModulesMainProject, srcMainProject, TaonGeneratedFiles, taonJsonMainProject, } from '../../../constants';
...
export class ArtifactManager {
    ...
    static for(project) {
        ...
        for (const key of Object.keys(artifactProcess)) {
            const instance = artifactProcess[___NS__camelCase(key)];
            // @ts-expect-error
    ...
    async init(initOptions) {
        ...
        if (this.project.framework.frameworkVersionLessThan('v18')) {
            ...
            Helpers__NS__error(`Please upgrade taon framework version to to at least v18
        ...
        if (!initOptions.init.struct) {
            ...
            if (config.frameworkName === tnpPackageName) {
                ...
                if (node_modules_path !== node_modules_real_path) {
                    ...
                    // );
                    const properRelativeNodeModulesPath = crossPlatformPath(path.resolve(config.dirnameForTnp, this.project.ins.taonProjectsRelative, `${containerPrefix}${this.project.taonJson.frameworkVersion}`, nodeModulesMainProject));
    ...
    ${missingDependencies.map(d => `- ${chalk.bold(d)}`).join()}`, false, true);
            }
        }
        //#endregion
        ...
        Helpers__NS__removeFileIfExists(path.join(this.project.location, fileName.tnpEnvironment_json));
        if (!initOptions.isCiProcess && !this.project.framework.isCoreProject) {
            ...
            try {
                fse.unlinkSync(this.project.pathFor(`${distMainProject}/${nodeModulesMainProject}`));
            }
            catch (error) { }
            try {
                this.project
                    .run(`git rm -f ${dotVscodeMainProject}/${TaonGeneratedFiles.LAUNCH_JSON}`, {
                    output: false,
                    silence: true,
                })
                    .sync();
            ...
            this.project.removeFile(`${dotVscodeMainProject}/${TaonGeneratedFiles.LAUNCH_BACKUP_JSON}`);
            this.project.removeFile('run-org.js');
            if (this.project.typeIs(LibTypeEnum.CONTAINER)) {
                this.project.removeFile(`${srcMainProject}/${TaonGeneratedFiles.VARS_SCSS}`);
            }
            this.project.remove(`${srcMainProject}/docker`, true);
            if (this.project.hasFolder([srcMainProject, migrationsFromLib]) &&
                !this.project.hasFolder([srcMainProject, libFromSrc, migrationsFromLib])) {
                HelpersTaon__NS__copy(this.project.pathFor([srcMainProject, migrationsFromLib]), this.project.pathFor([srcMainProject, libFromSrc, migrationsFromLib]), { recursive: true, overwrite: true });
                Helpers__NS__remove(this.project.pathFor([srcMainProject, migrationsFromLib]));
            }
            try {
                this.project
                    .run(`git rm -f ${dotVscodeMainProject}/${TaonGeneratedFiles.LAUNCH_BACKUP_JSON}`, {
                    output: false,
                    silence: true,
                })
                    .sync();
    ...
    async build(buildOptions) {
        ...
        `, false, true);
            ...
            while (this.project.framework.frameworkVersionLessThan(CURRENT_PACKAGE_VERSION.split('.')[0])) {
    ...
    async release(releaseOptions, autoReleaseProcess = false) {
        ...
        if (releaseOptions.release.askUserBeforeFinalAction) {
            await this.project.releaseProcess.checkBundleQuestion(this.project.location, `[${releaseOptions.release.releaseType}] Check ${chalk.bold('code')} before final action`);
        }
        if (!releaseOptions.release.skipTagGitPush) {
            if (!releaseOptions.release.autoReleaseUsingConfig) {
                Helpers__NS__info(`Checking project path "${releaseOutput.releaseProjPath}" ..`);
                await this.project.releaseProcess.checkBundleQuestion(releaseOutput.releaseProjPath, `[${releaseOptions.release.releaseType}] Check ${chalk.bold('bundled code')} before tagging/pushing`);
            }
            releaseOutput.projectsReposToPush =
                releaseOutput.projectsReposToPush || [];
            for (const repoAbsPath of Utils__NS__uniqArray(releaseOutput.projectsReposToPush)) {
                Helpers__NS__info(`Checking  path "${repoAbsPath}" `);
                if (!releaseOptions.release.autoReleaseUsingConfig) {
                    await this.project.releaseProcess.checkBundleQuestion(repoAbsPath, `[${releaseOptions.release.releaseType}] Check ${chalk.bold('project repo')} before pushing`);
                }
            ...
            for (const repoAbsPath of Utils__NS__uniqArray(releaseOutput.projectsReposToPushAndTag)) {
                if (!releaseOptions.release.autoReleaseUsingConfig) {
                    Helpers__NS__info(`Checking  path "${repoAbsPath}" ..`);
                    await this.project.releaseProcess.checkBundleQuestion(repoAbsPath, `[${releaseOptions.release.releaseType}] Check ${chalk.bold('project repo')} before tagging/pushing`);
                }
                await HelpersTaon__NS__git__NS__tagAndPushToGitRepo(repoAbsPath, {
                    newVersion: releaseOutput.resolvedNewVersion,
                    autoReleaseUsingConfig: releaseOptions.release.autoReleaseUsingConfig,
    ...
    async releaseAllChildren(options, children = this.project.children) {
        ...
        for (let index = 0; index < children.length; index++) {
            ...
            Helpers__NS__info(`Releasing container child: ${child.name}  (${howManyChildren}/${index + 1}) `);
    ...
    async tryCatchWrapper(action, actionName, project = this.project) {
        ...
        while (true) {
            try {
                ...
                console.error(error);
                Helpers__NS__error(`Not able to ${actionName} your project ${chalk.bold(project.genericName)}`, true, true);
                const errorOptions = {
...
//# sourceMappingURL=c:/Users/darek/projects/npm/taon-dev/tnp/dist/lib-prod/project/abstract/artifacts/artifacts-manager.js.map