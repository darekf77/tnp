...
import { BasePackageJson, Helpers__NS__createSymLink, Helpers__NS__readFile, Helpers__NS__readJson, Helpers__NS__remove, Helpers__NS__writeFile, Helpers__NS__writeJson, HelpersTaon__NS__move } from 'tnp-helpers/lib-prod';
import { templateFolderForArtifact } from '../../../../../app-utils';
import { browserMainProject, CoreNgTemplateFiles, distFromNgBuild, distMainProject, libFromSrc, myLibFromNgProject, packageJsonNgProject, prodSuffix, projectsFromNgTemplate, srcMainProject, tmpLibsForDist, tmpLibsForDistWebsql, tmpSrcDist, tmpSrcDistWebsql, tsconfigNgProject, tsconfigSpecNgProject, websqlMainProject, } from '../../../../../constants';
...
export class InsideStructAngularLib extends BaseInsideStruct {
    ...
    insideStruct() {
        ...
        const result = InsideStruct.from({
            ...
            pathReplacements: [
                [
                    new RegExp(`^${Utils__NS__escapeStringForRegEx(templateFolderForArtifact(this.getCurrentArtifact()) + '/')}`),
                    () => {
            ...
            endAction: ({ replacement }) => {
                ...
                (() => {
                    const jsonPath = path.join(this.project.location, replacement(tmpProjectsStandalone), packageJsonNgProject);
                ...
                (() => {
                    const source = path.join(this.project.location, replacement(tmpProjectsStandalone), `${projectsFromNgTemplate}/${myLibFromNgProject}`);
                    const dest = path.join(this.project.location, replacement(tmpProjectsStandalone), `${projectsFromNgTemplate}/${this.project.name}`);
                    Helpers__NS__remove(dest);
                    HelpersTaon__NS__move(source, dest);
                })();
                //#endregion
                (() => {
                    ...
                    const source = path.join(this.project.location, browserTsCode, libFromSrc);
                    const dest = path.join(this.project.location, replacement(tmpProjectsStandalone), `${projectsFromNgTemplate}/${this.project.name}/${srcMainProject}/${libFromSrc}`);
                    Helpers__NS__remove(dest);
                    ...
                    const sourcePublicApi = path.join(this.project.location, replacement(tmpProjectsStandalone), `${projectsFromNgTemplate}/${this.project.name}/${srcMainProject}/${fileName.public_api_ts}`);
                    let publicApiFile = Helpers__NS__readFile(sourcePublicApi);
                    const sourceTsconfig = path.join(this.project.location, replacement(tmpProjectsStandalone), tsconfigSpecNgProject);
                    let tsconfigJson = Helpers__NS__readJson(sourceTsconfig, void 0, true);
                    // console.log({
                    //   sourceTsconfig
                    // })
                    if (tsconfigJson) {
...
`.trimLeft();
                    if (tsconfigJson) {
                        Helpers__NS__writeJson(sourceTsconfig, tsconfigJson);
                    }
                    Helpers__NS__writeFile(sourcePublicApi, publicApiFile);
                })();
                ...
                [libPackageJson].forEach(f => {
                    ...
                    content = content.replace(new RegExp(Utils__NS__escapeStringForRegEx(myLibFromNgProject), 'g'), `${this.project.name}` +
                ...
                (() => {
                    const json = Helpers__NS__readJson(ngPackageJson); // dist is on purpose
                    json.dest = json.dest.replace(`/${distFromNgBuild}/${this.project.name}`, `/../../${distMainProject +
                        (this.initOptions.build.prod ? `${prodSuffix}` : '')}/` +
                        `${this.initOptions.build.websql ? websqlMainProject : browserMainProject}`);
                    Helpers__NS__writeJson(ngPackageJson, json);
                })();
            },
        }, project);
        return result;
...
//#endregion
//# sourceMappingURL=c:/Users/darek/projects/npm/taon-dev/tnp/dist/lib-prod/project/abstract/artifacts/npm-lib-and-cli-tool/tools/inside-struct-angular-lib.js.map