...
> {
  ...
  get projectToCopyTo() {
    ...
    return Utils__NS__uniqArray<Project>(result, 'location' as keyof Project);
    //#endregion
  }
  //#endregion
    ...
    if (projectToCopyTo.length > 0) {
      const porjectINfo =
        projectToCopyTo.length === 1
          ? `project "${(___NS__first(projectToCopyTo as any) as Project).name}"`
      ...
      Helpers__NS__info(`[buildable-project] copying compiled code/assets to ${
