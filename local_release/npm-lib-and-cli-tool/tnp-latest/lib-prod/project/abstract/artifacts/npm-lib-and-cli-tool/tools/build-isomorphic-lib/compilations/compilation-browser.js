//#region imports
import { BaseClientCompiler } from 'incremental-compiler/lib-prod';
import { extForSassLikeFiles } from 'tnp-core/lib-prod';
import { TAGS } from 'tnp-core/lib-prod';
import { path, fse, crossPlatformPath } from 'tnp-core/lib-prod';
import { Helpers__NS__mkdirp, Helpers__NS__removeFileIfExists, Helpers__NS__removeFolderIfExists, Helpers__NS__warn, HelpersTaon__NS__copyFile } from 'tnp-helpers/lib-prod';
import { assetsFromTempSrc, distMainProject, DS_Store, prodSuffix, tmpSourceDist, tmpSrcDist, tmpSrcDistWebsql, } from '../../../../../../../constants';
...
export class BrowserCompilation extends BaseClientCompiler {
    ...
    srcFolder, buildOptions) {
        ...
        this.absPathTmpSrcDistFolderWEBSQL = crossPlatformPath(path.join(this.project.location || '', tmpSrcDistWebsql + (buildOptions.build.prod ? prodSuffix : '')));
        this.absPathTmpSrcDistFolderNORMAL = crossPlatformPath(path.join(this.project.location || '', tmpSrcDist + (buildOptions.build.prod ? prodSuffix : '')));
    }
    //#endregion
    //#endregion
    //#region methods
    ...
    async syncAction(absFilesFromSrc) {
        ...
        this.codecutNORMAL = new CodeCut(this.absPathTmpSrcDistFolderNORMAL, {
            replacements: tagsNormal,
            env: buildOptForNormal,
        }, this.project, buildOptForNormal);
        this.codecutWEBSQL = new CodeCut(this.absPathTmpSrcDistFolderWEBSQL, {
            replacements: tagsWebsql,
            env: buildOptForWebsql,
        }, this.project, buildOptForWebsql);
        //#endregion
        //#region prepare tmp folders
        const tmpSource = this.project.pathFor(tmpSourceDist + isProd ? prodSuffix : '');
        Helpers__NS__removeFolderIfExists(this.absPathTmpSrcDistFolderWEBSQL);
        Helpers__NS__removeFolderIfExists(this.absPathTmpSrcDistFolderNORMAL);
        Helpers__NS__mkdirp(this.absPathTmpSrcDistFolderNORMAL);
        Helpers__NS__mkdirp(this.absPathTmpSrcDistFolderWEBSQL);
        Helpers__NS__mkdirp(tmpSource);
        Helpers__NS__removeFolderIfExists(tmpSource);
        //#endregion
        ...
        const relativePathesToProcess = absFilesFromSrc.map(absFilePath => {
            ...
            const isScssOrSass = extForSassLikeFiles.includes(path.extname(path.basename(relativePath)));
            if (isScssOrSass) {
                const destScss = this.sassDestFor(relativePath);
                HelpersTaon__NS__copyFile(absFilePath, destScss);
            }
            return relativePath;
        });
    ...
    sassDestFor(relativePath) {
        //#region @backendFunc
        const destScss = this.project.pathFor(`${distMainProject}/${path
    ...
    async asyncActionFor(event, websql) {
        ...
        const relativeFilePath = crossPlatformPath(absoluteFilePath.replace(`${crossPlatformPath(path.join(this.project.location, this.srcFolder))}/`, ''));
        ...
        if (!websql) {
            ...
            (() => {
                ...
                if (event.eventName === 'unlinkDir') {
                    Helpers__NS__removeFolderIfExists(destinationFileBackendPath);
                }
                else {
                    if (event.eventName === 'unlink') {
                        if (relativeFilePath.startsWith(`${assetsFromTempSrc}/`)) {
                            ...
                            try {
                                Helpers__NS__removeFileIfExists(destinationFileBackendPath);
                            }
                            catch (error) {
                                Helpers__NS__warn(`Error during removing file ${destinationFileBackendPath}`);
                            }
                        }
                    }
                    else {
            ...
            //#region scss file
            (() => {
                const isScssOrSass = extForSassLikeFiles.includes(path.extname(path.basename(relativeFilePath)));
                if (!isScssOrSass) {
                    return;
                }
                ...
                if (event.eventName === 'unlinkDir') {
                    try {
                        Helpers__NS__removeFolderIfExists(destinationFileScssPath);
                    }
                    catch (error) {
                        Helpers__NS__warn(`Error during removing folder ${destinationFileScssPath}`);
                    }
                }
                else {
                    if (event.eventName === 'unlink') {
                        if (relativeFilePath.startsWith(`${assetsFromTempSrc}/`)) {
                            ...
                            try {
                                Helpers__NS__removeFileIfExists(destinationFileScssPath);
                            }
                            catch (error) {
                                Helpers__NS__warn(`Error during removing file ${destinationFileScssPath}`);
                            }
                        }
                    }
                    else {
                        if (fse.existsSync(absoluteFilePath)) {
                            ...
                            //#endregion
                            //#region remove deist if directory
                            if (fse.existsSync(destinationFileScssPath) &&
                                fse.lstatSync(destinationFileScssPath).isDirectory()) {
                                fse.removeSync(destinationFileScssPath);
                            }
                            //#endregion
                            HelpersTaon__NS__copyFile(absoluteFilePath, destinationFileScssPath);
                        }
                    }
                }
            })();
        ...
        //#region browser file
        (() => {
            const destinationFilePath = crossPlatformPath(path.join(this.project.location, websql ? tmpSrcDistWebsql : tmpSrcDist, // prod not need for async
            relativeFilePath));
            if (event.eventName === 'unlinkDir') {
                Helpers__NS__removeFolderIfExists(destinationFilePath);
            }
            else {
                if (event.eventName === 'unlink') {
                    if (relativeFilePath.startsWith(`${assetsFromTempSrc}/`)) {
                        ...
                        try {
                            Helpers__NS__removeFileIfExists(destinationFilePath);
                        }
                        catch (error) {
                            Helpers__NS__warn(`Error during removing file ${destinationFilePath}`);
                        }
                    }
                }
                else {
...
//# sourceMappingURL=c:/Users/darek/projects/npm/taon-dev/tnp/dist/lib-prod/project/abstract/artifacts/npm-lib-and-cli-tool/tools/build-isomorphic-lib/compilations/compilation-browser.js.map