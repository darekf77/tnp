//#region imports
import { config, folderName, UtilsFilesFoldersSync__NS__copy, UtilsFilesFoldersSync__NS__copyFile, UtilsFilesFoldersSync__NS__filterDontCopy, UtilsFilesFoldersSync__NS__filterOnlyCopy, UtilsFilesFoldersSync__NS__getFilesFrom, UtilsFilesFoldersSync__NS__getFoldersFrom, UtilsFilesFoldersSync__NS__IGNORE_FOLDERS_FILES_PATTERNS, UtilsFilesFoldersSync__NS__move, UtilsFilesFoldersSync__NS__readFile, UtilsFilesFoldersSync__NS__UtilsFilesFoldersSyncGetFilesFromOptions, UtilsFilesFoldersSync__NS__writeFile, UtilsJson__NS__AttrJsoncProp, UtilsJson__NS__getAtrributiesFromJsonWithComments, UtilsJson__NS__getAttributiesFromComment, UtilsJson__NS__readJson, UtilsJson__NS__readJsonWithComments } from 'tnp-core/lib-prod';
...
export class BackendCompilation {
  ...
  public reExportsForPackagesLib: Map<
    string,
    UtilsTypescript__NS__GatheredExportsMap
  >;
  ...
  async syncAction(filesPathes: string[]) {
    ...
    if (!this.buildOptions.build.watch) {
      ...
      this.project.packagesRecognition.allIsomorphicPackagesFromMemory.forEach(
        pkgName => {
          const namespacesLib = UtilsJson__NS__readJson(
          ...
          const reExportsLib = UtilsJson__NS__readJson(
            this.project.nodeModules.pathFor([
              pkgName,
          ...
          const namespacesBrowser = UtilsJson__NS__readJson(
            this.project.nodeModules.pathFor([
              pkgName,
          ...
          const reExportsBrowser = UtilsJson__NS__readJson(
            this.project.nodeModules.pathFor([
              pkgName,
          ...
          const namespacesWebsql = UtilsJson__NS__readJson(
            this.project.nodeModules.pathFor([
              pkgName,
          ...
          const reExportsWebsql = UtilsJson__NS__readJson(
            this.project.nodeModules.pathFor([
              pkgName,
    ...
    if (!this.buildOptions.build.watch && this.buildOptions.build.prod) {
      //#region detect files
      const filesBrowser = UtilsFilesFoldersSync__NS__getFilesFrom(
      ...
      const filesWebsql = UtilsFilesFoldersSync__NS__getFilesFrom(
      ...
      const filesLib = UtilsFilesFoldersSync__NS__getFilesFrom(
    ...
    if (!this.buildOptions.build.watch && this.buildOptions.build.prod) {
      ...
      UtilsFilesFoldersSync__NS__getFilesFrom(
        this.project.pathFor([distMainProject + prodSuffix]),
        {
          recursive: true,
          followSymlinks: false,
  ...
  private setGeneratedReExportsToMapForCurrentPackage(
    reExportForPackages: Map<string, UtilsTypescript__NS__GatheredExportsMap>,
    folderInDist:
  ...
  private saveGenerateReExportsIndProdDistForCurrentPackage(
    ...
    namespacesForPackages: Map<string, UtilsTypescript__NS__SplitNamespaceResult>,
    folderInDist:
      | typeof libFromCompiledDist
      | typeof browserFromCompiledDist
      | typeof websqlFromCompiledDist,
    ...
    const data = UtilsTypescript__NS__gatherExportsMapFromIndex(
  ...
  private combineNamespacesForCurrentPackage(
    ...
    const data = files
      ...
      .reduce((a, b) => {
        const jsonMap: UtilsTypescript__NS__SplitNamespaceResult =
          UtilsJson__NS__readJson(b) || {};
        return ___NS__merge(a, jsonMap);
      }, {} as UtilsTypescript__NS__SplitNamespaceResult);
  ...
  private productionCodeReplacement(
    files: string[],
    namespacesForPackages: Map<string, UtilsTypescript__NS__SplitNamespaceResult>,
    reExports: Map<string, UtilsTypescript__NS__GatheredExportsMap>,
  ): void {
    //#region @backendFunc
    files
      .filter(f => f.endsWith('.ts') || f.endsWith('.tsx'))
      .forEach(f => {
        ...
        UtilsTypescript__NS__updateSplitNamespaceReExports(
          namespacesForPackages,
          reExports,
        );
        ...
        const renamedImportsOrExports =
          UtilsTypescript__NS__extractRenamedImportsOrExport(content).map(c => {
            ...
            for (const renamed of renamedImportsOrExports) {
              if (
                ...
                namespacesForPackages.set(
                  libName,
                  UtilsTypescript__NS__updateSplitNamespaceResultMapReplaceObj(
                    namespaceData,
        ...
        namespacesForPackages.forEach((namespaceData, libName) => {
          ...
          content = UtilsTypescript__NS__replaceNamespaceWithLongNames(
            content,
  ...
  async libCompilation(
    ...
    if (!this.isEnableCompilation) {
      Helpers__NS__log(
        `Compilation disabled for ${___NS__startCase(BackendCompilation.name)}`,
      );
      return;
    }
    const paramasCommon = {
