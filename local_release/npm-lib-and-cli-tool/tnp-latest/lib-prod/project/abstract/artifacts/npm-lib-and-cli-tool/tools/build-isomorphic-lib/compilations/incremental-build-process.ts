//#region imports
import { StartAndWatchOptions } from 'incremental-compiler/lib-prod';
import { config } from 'tnp-core/lib-prod';
import { path, crossPlatformPath } from 'tnp-core/lib-prod';
...
export class IncrementalBuildProcess {
  ...
  private recreateBrowserLinks(bc: BrowserCompilation) {
    ...
    // );
    // Helpers__NS__log(`recreateBrowserLinks: outDistPath: ${outDistPath}`);
    // Helpers__NS__removeFolderIfExists(outDistPath);
    // const targetOut = crossPlatformPath(
    //   path.join(bc.project.location, bc.backendOutFolder, 'dist'),
    // );
    // Helpers__NS__log(`recreateBrowserLinks: targetOut: ${targetOut}`);
    // Helpers__NS__createSymLink(targetOut, outDistPath, {
  ...
  async start(taskName?: string, afterInitCallBack?: () => void) {
    ...
    if (___NS__isFunction(afterInitCallBack)) {
      await Helpers__NS__runSyncOrAsync({ functionFn: afterInitCallBack });
    }
    //#endregion
