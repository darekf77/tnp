  ...
  extends BaseFeatureForProject<Project>
{
  fileCommand(files: string[]) {
    ...
    const useFiles = ___NS__isArray(files) && files.length > 0;
    const ext =
      files.length > 1 || !___NS__first(files).endsWith('.test.ts')
        ? '*.test.ts'
        : '';
    const res = `${useFiles ? `src/tests/**/*${files.length === 1 ? `${___NS__first(files)}` : `(${files.join('|')})`}${ext}` : 'src/**/*.test.ts'}`;
    return res;
  }
