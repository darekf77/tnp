...
> {
  ...
  public writeDataIntoAppTs() {
    ...
    appFileContent = UtilsTypescript__NS__removeTaggedImportExport(
      appFileContent,
      [TAGS.APP_TS_GENERATED],
      true,
    );

    appFileContent = UtilsTypescript__NS__removeTaggedArrayObjects(
      appFileContent,
      [TAGS.APP_TS_GENERATED],
      true,
    );

    appFileContent = UtilsTypescript__NS__removeTaggedLines(
      appFileContent,
      [TAGS.APP_TS_GENERATED],
      true,
    );
    ...
    appFileContent = UtilsTypescript__NS__addBelowPlaceholder(
      appFileContent,
    ...
    const generatedInitContextFunctions = this.contextsRelativePaths.map(r => {
      const baseName = path.basename(r);
      const cleanBasename = baseName.replace('.ts', '').replace('.tsx', '');
      return `await ${___NS__upperFirst(___NS__camelCase(cleanBasename))}.initialize(); // ${TAGS.APP_TS_GENERATED}`;
    });
