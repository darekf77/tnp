...
export class $Init extends BaseCli {
  ...
  private async __askForWhenEmpty(): Promise<void> {
    ...
    let proj: Project;
    const yesNewProj = await Helpers__NS__questionYesNo(
      `Do you wanna init project in this folder ?`,
    );
    if (yesNewProj) {
      const responseProjectType = // @ts-ignore
        await Helpers.autocompleteAsk<CoreModels__NS__LibType>(
      ...
      if (responseProjectType === LibTypeEnum.CONTAINER) {
        ...
        } as Partial<Models__NS__TaonJsonContainer>);
      } else {
        Helpers__NS__writeFile(
          [crossPlatformPath(this.cwd), packageJsonMainProject],
          {
        ...
        Helpers__NS__writeFile([crossPlatformPath(this.cwd), taonJsonMainProject], {
          ...
          version: DEFAULT_FRAMEWORK_VERSION, // OK
        } as Partial<Models__NS__TaonJsonStandalone>);
      }
