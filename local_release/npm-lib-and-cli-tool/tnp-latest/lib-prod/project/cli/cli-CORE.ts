...
export class $Core extends BaseCli {
  ...
  async createNext(): Promise<void> {
    ...
    const latestCoreContainerVersion = ___NS__first(
      Helpers__NS__foldersFrom(coreContainerParentLocation)
    ...
    await newContainer.framework.setFrameworkVersion(
      `v${newVersionOfCore}` as CoreModels__NS__FrameworkVersion,
    );
  ...
  async setFrameworkVersion(): Promise<void> {
    const newFrameworkVersion =
      `v${this.firstArg.replace('v', '')}` as CoreModels__NS__FrameworkVersion;
