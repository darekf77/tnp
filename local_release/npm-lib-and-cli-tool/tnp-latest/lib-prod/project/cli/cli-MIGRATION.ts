...
class $Migration extends BaseCli {
  ...
  async _displayMenu() {
    ...
    const resp = await UtilsTerminal__NS__select<keyof typeof options>({
      choices: options,
    ...
    if (resp === 'create') {
      ...
      const allMigrationsOptions = Helpers__NS__getFilesFrom(
