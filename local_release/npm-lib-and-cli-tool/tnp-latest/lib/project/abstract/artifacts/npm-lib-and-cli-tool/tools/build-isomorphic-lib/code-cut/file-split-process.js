...
exports.SplitFileProcess = void 0;
const lib_1 = require("tnp-core/lib");
const lib_2 = require("tnp-core/lib");
const lib_3 = require("tnp-helpers/lib");
const constants_1 = require("../../../../../../../constants");
const code_split_process_enum_1 = require("./code-split-process.enum");
class SplitFileProcess {
    fileContent;
    filePath;
    isomorphicLibraries;
    currentProjectName;
    ...
    constructor(fileContent, filePath, isomorphicLibraries, currentProjectName, currentProjectNpmName) {
        ...
        this._importExports =
            lib_3.UtilsTypescript.recognizeImportsFromContent(fileContent);
        this.processImportsExports();
    }
    //#region get content
    get content() {
        //#region @backendFunc
        if (lib_2._.isUndefined(lib_1.extAllowedToExportAndReplaceTSJSCodeFiles.find(a => this.filePath.endsWith(a)))) {
        ...
        const BEFORE_PROCESSES = Object.values(code_split_process_enum_1.CodeSplitProcess.Before.Split.ImportExport);
        for (const imp of this._importExports) {
            if (!imp.isIsomorphic) {
                continue;
            }
            for (const processFun of BEFORE_PROCESSES) {
                if (lib_2._.isFunction(processFun)) {
    ...
    processImportsExports() {
        for (const imp of this._importExports) {
            ...
            const matchRegex = new RegExp(`^(${this.isomorphicLibraries
                ...
                .map(a => lib_2.Utils.escapeStringForRegEx(a))
                .join('|')})`);
            const match = imp.cleanEmbeddedPathToFile.match(matchRegex);
            // console.log(`match: >>${matchRegex.source}<< for >>${imp.embeddedPathToFile}<<`);
            const isDummyLib = imp.cleanEmbeddedPathToFile.startsWith(`${constants_1.DUMMY_LIB}/`) ||
                imp.cleanEmbeddedPathToFile === constants_1.DUMMY_LIB;
            imp.isIsomorphic =
                (Array.isArray(match) && match.length > 0) || isDummyLib;
            if (imp.isIsomorphic) {
                if (isDummyLib) {
                    imp.packageName = constants_1.DUMMY_LIB;
                }
                else {
                    imp.packageName = lib_2._.first(match);
                }
    ...
    replaceInFile(fileContent, imports, debug = false) {
        ...
        for (const imp of imports) {
            ...
            if (prevLine && prevLine.includes(constants_1.taonIgnore)) {
                continue;
            }
...
exports.SplitFileProcess = SplitFileProcess;
//# sourceMappingURL=c:/Users/darek/projects/npm/taon-dev/tnp/dist/lib/project/abstract/artifacts/npm-lib-and-cli-tool/tools/build-isomorphic-lib/code-cut/file-split-process.js.map