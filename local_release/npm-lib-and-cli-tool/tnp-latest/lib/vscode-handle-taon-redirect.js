...
exports.handleTaonRedirect = handleTaonRedirect;
...
function handleTaonRedirect(context, vscode) {
    ...
    vscode.window.onDidChangeActiveTextEditor(async (editor) => {
        ...
        let originalPath = (0, lib_1.crossPlatformPath)(document.uri.fsPath);
        ...
        if (match2) {
            const toRep = (0, lib_1.crossPlatformPath)(lib_1.path.dirname(match2[0]));
            const [porjPath] = originalPath.split(toRep);
            // console.log({ match2, porjPath, toRep });
            if (await lib_1.fse.exists((0, lib_1.crossPlatformPath)([porjPath, toRep, 'source']))) {
                originalPath = (0, lib_1.crossPlatformPath)([porjPath, toRep, 'source/index.ts']);
            }
        }
        // else {
        ...
        if (match) {
            // /node_modules/taon/source/ zero
            const [porjPath, relative] = originalPath.split(match[0]);
            let fullPathToSource = (0, lib_1.crossPlatformPath)([porjPath, match[0]]);
            try {
                fullPathToSource = (0, lib_1.crossPlatformPath)(lib_1.fse.realpathSync(fullPathToSource));
            }
            catch (error) {
                // console.log('failed to read symlink');
                return;
            }
            const properRedirectLink = (0, lib_1.crossPlatformPath)([
                fullPathToSource,
                relative,
            ]);
...
//# sourceMappingURL=c:/Users/darek/projects/npm/taon-dev/tnp/dist/lib/vscode-handle-taon-redirect.js.map