...
exports.vscodeExtMethods = void 0;
const lib_1 = require("tnp-core/lib");
const framework_utils_1 = require("./framework-utils");
const project_1 = require("./project/abstract/project");
const vscodeExtMethods = (FRAMEWORK_NAME) => {
    ...
    // const groupOpen = `${toolName} open`;
    const taonHomeDirForCopyPathJson = (0, lib_1.crossPlatformPath)([
        lib_1.UtilsOs.getRealHomeDir(),
        `.taon/vscode-copy-buffor/path.json`,
    ]);
    if (!lib_1.Helpers.exists(lib_1.path.dirname(taonHomeDirForCopyPathJson))) {
        lib_1.Helpers.mkdirp(lib_1.path.dirname(taonHomeDirForCopyPathJson));
    }
    if (!lib_1.Helpers.exists(taonHomeDirForCopyPathJson)) {
        lib_1.Helpers.writeJson(taonHomeDirForCopyPathJson, {});
    }
    //#region copy or cut project
    const copyOrCutProject = async (action, vscode, uri) => {
        //#region @backendFunc
        const WORKSPACE_MAIN_FOLDER_PATH = (0, lib_1.crossPlatformPath)(uri.path);
        const nearestProject = project_1.Project.ins.From(WORKSPACE_MAIN_FOLDER_PATH);
        if (!nearestProject) {
            vscode.window.showErrorMessage(`Cannot find project nearest project in path ${WORKSPACE_MAIN_FOLDER_PATH}`);
            return;
        }
        if (!lib_1.Helpers.exists(taonHomeDirForCopyPathJson)) {
            lib_1.Helpers.writeJson(taonHomeDirForCopyPathJson, {});
        }
        const currentContent = lib_1.Helpers.readJson(taonHomeDirForCopyPathJson);
        const resultContent = lib_1._.merge(currentContent, {
        ...
        lib_1.Helpers.writeJson(taonHomeDirForCopyPathJson, resultContent);
        vscode.window.showInformationMessage(`
            Path ${nearestProject.location} saved ${lib_1._.startCase(action)?.toLowerCase()} .
            `);
        //#endregion
    };
    //#endregion
    return [
        ...
        //#region PASTE PROJECT
        {
            ...
            async exec({ vscode, uri, rootFolderPath }) {
                //#region @backendFunc
                let MAIN_CLICKED_PATH = (0, lib_1.crossPlatformPath)(uri.path) || (0, lib_1.crossPlatformPath)(rootFolderPath);
                if (!MAIN_CLICKED_PATH) {
                    return;
                }
                if (!lib_1.Helpers.isFolder(MAIN_CLICKED_PATH)) {
                    MAIN_CLICKED_PATH = (0, lib_1.crossPlatformPath)(lib_1.path.dirname(MAIN_CLICKED_PATH));
                }
                if (!lib_1.Helpers.exists(taonHomeDirForCopyPathJson)) {
                    lib_1.Helpers.writeJson(taonHomeDirForCopyPathJson, {});
                }
                const currentContent = lib_1.Helpers.readJson(taonHomeDirForCopyPathJson);
                const copyOrMove = async (action) => {
                    const copyOrMoveSource = project_1.Project.ins.nearestTo(currentContent[action]);
                    if (!copyOrMoveSource) {
                        vscode.window.showErrorMessage(`Cannot find project in path ${currentContent[action]}`);
                        return;
                    }
                    if (action === 'toMove') {
                        await copyOrMoveSource.fileFoldersOperations.moveProjectTo(MAIN_CLICKED_PATH);
                    }
                    else if (action === 'toCopy') {
                        await copyOrMoveSource.fileFoldersOperations.copyProjectTo(MAIN_CLICKED_PATH);
                    }
                    else {
                    ...
                    const mainProj = project_1.Project.ins.From(MAIN_CLICKED_PATH);
                    if (mainProj) {
                        try {
                    ...
                    const destProj = project_1.Project.ins.From([
                        MAIN_CLICKED_PATH,
        ...
        //#region OPEN WORKBENCH PATH
        {
            ...
            async exec({ vscode, uri }) {
                ...
                let stat = lib_1.fse.lstatSync(fsPath);
                if (stat.isSymbolicLink()) {
                    fsPath = lib_1.fse.realpathSync(fsPath);
                }
                stat = lib_1.fse.lstatSync(fsPath);
                if (!stat.isDirectory()) {
                    fsPath = lib_1.path.dirname(fsPath);
                }
                // Resolve symlinks (real path)
                const finalUri = vscode.Uri.file(fsPath);
                await vscode.commands.executeCommand('vscode.openFolder', finalUri, true);
            },
            options: {
        ...
        //#region CREATE MIGRATION
        {
            ...
            options: {
                ...
                findNearestProjectType: lib_1.LibTypeEnum.ISOMORPHIC_LIB,
                resolveVariables: [
        ...
        },
        {
            ...
            exec: ({ vscode, uri, cwd }) => {
                const WORKSPACE_MAIN_FOLDER_PATH = (0, lib_1.crossPlatformPath)(uri.path);
                vscode.env.clipboard.writeText(WORKSPACE_MAIN_FOLDER_PATH.replace(cwd + '/', ''));
            },
            options: {
                cancellable: false,
                showSuccessMessage: false,
        ...
        },
        {
            ...
            exec: ({ vscode, uri, cwd }) => {
                const WORKSPACE_MAIN_FOLDER_PATH = (0, lib_1.crossPlatformPath)(uri.path);
                vscode.env.clipboard.writeText(lib_1.path.basename(WORKSPACE_MAIN_FOLDER_PATH));
            },
            options: {
                cancellable: false,
                showSuccessMessage: false,
        ...
        //#region GENERATE index.ts in selected folder
        {
            ...
            exec: async (data) => {
                ...
                vscode.env.clipboard.writeText(await framework_utils_1.FrameworkUtils.copyTree(WORKSPACE_MAIN_FOLDER_PATH));
            },
            options: {
                titleWhenProcessing: 'copying files and folders to clipboard MD/AI ready format',
                showSuccessMessage: false,
            },
        },
        {
            ...
            exec: async (data) => {
                ...
                const arr = (selectedUris || []).map(c => (0, lib_1.crossPlatformPath)(c.path));
                // console.log({ arr });
                const WORKSPACE_MAIN_FOLDER_PATH = (0, lib_1.crossPlatformPath)(uri.path);
                // vscode.window.showInformationMessage(WORKSPACE_MAIN_FOLDER_PATH);
                vscode.env.clipboard.writeText(await framework_utils_1.FrameworkUtils.copyToAiContent(WORKSPACE_MAIN_FOLDER_PATH, arr));
            },
            options: {
                titleWhenProcessing: 'copying files and folders to clipboard MD/AI ready format',
                showSuccessMessage: false,
        ...
        },
        {
            ...
            exec: async ({ selectedUris, uri, vscode }) => {
                ...
                const processed = await framework_utils_1.FrameworkUtils.pasteFromAIMDContentToFiles(WORKSPACE_MAIN_FOLDER_PATH, textFromClipboard);
                const nearestProject = project_1.Project.ins.nearestTo(WORKSPACE_MAIN_FOLDER_PATH);
                if (nearestProject) {
            ...
            options: {
                titleWhenProcessing: 'pasting from md/ai content to files and folders',
                showSuccessMessage: false,
            },
        },
        //#endregion
        ...
        if (!c.command) {
            c.command = `extension.${FRAMEWORK_NAME}.${lib_1.Utils.camelize(c.title)}`;
        }
...
exports.vscodeExtMethods = vscodeExtMethods;
//# sourceMappingURL=c:/Users/darek/projects/npm/taon-dev/tnp/dist/lib/vscode-ext-methods.js.map